...
| 6 | Approval gates on high-impact actions | **PARTIAL** | `agent_gateway.py:63-69` defines all 5 `APPROVAL_REQUIRED_ACTIONS`. `agent_gateway.py:129-136` checks the gate. | `test_gateway.py:73-89` tests `responder_card` and `stakeholder_update` (2 of 5) | **Gate is soft**: sets `allowed=True` with reason annotation, does not actually block. `resolve_incident` is NOT gated — invariant 5 ("no auto-resolve without authorized human") is prose-only. Only 2/5 actions tested. |
| 7 | Memory Bank cross-session recall | **PARTIAL** | `memory_bank.py:17-150` (store/find/outcomes). `learning/tools.py:15-50` (`find_similar_incidents`). 5 seed lessons (`:154-225`, labeled "pre-seeded" at `:152`). 2 outcomes (`:235-256`). | `test_memory_bank.py:16-86` (pre-seeded lessons, find-by-type, find-by-tags, outcome stats, similar incidents) | **No citation/confidence** in recall output. Recall is simple type/facility filtering, not semantic similarity. No cross-session test (lesson from incident A retrieved during incident B). |
...
| GAP-07 | Claim 7 | No citation/confidence in Memory Bank recall | Add `confidence` score (cosine/tag overlap) and `source` citation to `find_similar_incidents` return |
| GAP-08 | Claim 7 | No cross-session recall test | Add test: store lesson on incident A, retrieve on incident B with matching type |
